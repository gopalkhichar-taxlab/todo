 ...
 * Extended by TAL-90 to cover full CRUD + task counts.
 */
...
export function useUpdateStrategyMutation() {
  ...
  return useMutation<Strategy, ApiClientError, UpdateStrategyVariables>({
    ...
    onMutate: async ({ id, data }) => {
      // Cancel any in-flight queries for this strategy
      ...
      queryClient.setQueryData<Strategy[]>(strategiesKeys.list('active'), applyUpdate);
      queryClient.setQueryData<Strategy[]>(strategiesKeys.list('all'), applyUpdate);
      queryClient.setQueryData<Strategy>(strategiesKeys.detail(id), (prev) =>
        prev ? { ...prev, ...data } : prev,
      );
    ...
    onError: (_err, { id }, context) => {
      ...
      if (ctx.previousActive !== undefined) {
        queryClient.setQueryData(strategiesKeys.list('active'), ctx.previousActive);
      }
  ...
  return useMutation<Strategy, ApiClientError, string>({
    ...
    onError: (_err, _id, context) => {
      ...
      if (ctx.previousArchived !== undefined) {
        queryClient.setQueryData(strategiesKeys.list('archived'), ctx.previousArchived);
      }
      if (ctx.previousAll !== undefined) {
        queryClient.setQueryData(strategiesKeys.list('all'), ctx.previousAll);
      }
