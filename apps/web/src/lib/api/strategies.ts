 ...
 *
 * Extended by TAL-90 to cover the full CRUD surface plus task-count helpers.
 */
...
/** Flexible response shape — API may return an array or a page object. */
type ListStrategiesResponse = Strategy[] | { items: Strategy[]; nextCursor: string | null } | { data: Strategy[]; total: number };

// ---------------------------------------------------------------------------
// Task count shape returned by GET /tasks?strategy_id=&limit=0
// ---------------------------------------------------------------------------

export interface TaskCountDTO {
  /** Total tasks linked to the strategy */
  totalCount: number;
...
export async function listStrategies(
  ...
  const res = await apiRequest<ListStrategiesResponse>(`/strategies?status=${status}`);
 ...
 */
export async function getTaskCountsForStrategy(strategyId: string): Promise<TaskCountDTO> {
  try {
